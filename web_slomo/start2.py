...
class web_slomo:
    def __init__(self, sess):
        self.sess=sess
        self.slomo=slomo_model.Slomo_step2_LSTM(sess)

    def convert_fps(self,inpath, outpath, intercnt):
        ...
        return self.convert_mp4_h264(tep_path, outpath)
    ...
    def uploadfile(self):    
        ...
        if files is not None:
            filename=self.random_filename(files.filename)
            print (filename)
            ...
            oldfps=self.slomo.process_one_video( intercnt, uploadvideo_path, outpath, keep_shape=False, withtrain=True)
                
            #convert to h264
            outpath_h264=op.splitext(outpath)[0]+"_h264.mp4"
                
            outpath=self.convert_mp4_h264(outpath, outpath_h264)
                
            session['file_after']=outpath
            #convert fps
            uploadvideo_path_fps=op.splitext(uploadvideo_path)[0]+"_fps.mp4"
            uploadvideo_path=self.convert_fps(uploadvideo_path, uploadvideo_path_fps, intercnt)
